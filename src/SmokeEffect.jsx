  ...
  useEffect(() => {
    ...
    var GLProgram = (function () {
      function GLProgram(vertexShader, fragmentShader) {
        ...
        if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
          ...
          throw new Error(
            "SmokeEffect WebGL program link failed:\n" +
              (log || "No WebGL program error log available."),
          );
        }
    ...
    function compileShader(type, source) {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        ...
        throw new Error(
          "SmokeEffect WebGL shader compilation failed:\n" +
            (log || "No WebGL shader error log available."),
        );
      }
      return shader;
    }
    ...
    function update() {
      ...
      if (splatStack.length > 0) {
        for (var m = 0, stack = splatStack.pop(); m < stack; m++) {
          var color = [
            Math.random() * 10,
            Math.random() * 10,
            Math.random() * 10,
      ...
      for (var j = 0; j < config.PRESSURE_ITERATIONS; j++) {
        gl.bindTexture(gl.TEXTURE_2D, pressure.first[0]);
